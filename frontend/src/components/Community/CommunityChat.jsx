...
// ─── HELPERS ──────────────────────────────────────────────────────────────────
// FIX: colorFor uses email (from server's user.email), falls back to name
...
// resolveUser: user.name is the real username entered at registration — use as-is
const resolveUser = (u) => {
...
const parseMsg = (msg) => {
  ...
  // Extract image URL if encoded in text
  ...
  // Extract topic if encoded in text
  ...
  <div style={{ display:'flex', gap:4, alignItems:'center', padding:'8px 12px' }}>
    {[0,1,2].map(i => (
      <motion.div key={i}
        ...
        style={{ width:7, height:7, borderRadius:'50%', background:'#c4922a', opacity:0.7 }}
      />
    ))}
  </div>
);
...
// ─── AVATAR ───────────────────────────────────────────────────────────────────
// FIX: accepts name string, uses email for color if provided
        ...
        return (
          <motion.div key={r.id||i}
            ...
            <div style={{ flex:1 }}>
              ...
              {r.reactions?.length > 0 && <ReactionBar reactions={r.reactions} messageId={r.id} currentUserEmail={currentUserEmail} onReact={()=>{}} />}
            </div>
          </motion.div>
        );
      })}
...
const MessageCard = ({ msg, currentUserEmail, onReact, onReply, onDelete }) => {
  ...
  // FIX: resolve user object from server format {name, email}
    ...
    <motion.div layout initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }}
      ...
      <div ref={menuRef} onDoubleClick={() => setShowMenu(m => !m)}
        style={{
          ...
          background: isMe
            ? 'linear-gradient(135deg,rgba(196,146,42,0.12),rgba(155,107,47,0.08))'
            : 'linear-gradient(135deg,rgba(255,255,255,0.88),rgba(237,224,190,0.65))',
        ...
        {msg.image && (
          <img src={msg.image} alt="attached"
            style={{ display:'block', maxHeight:220, maxWidth:'100%', borderRadius:6, marginBottom:6, border:'1px solid rgba(196,146,42,0.2)' }} />
        )}

        {msg.text && (
          <p style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:15, color:'#353535', lineHeight:1.6, margin:0 }}>
            {msg.text}
          </p>
        )}
          ...
          {showMenu && (
            <motion.div
              ...
              style={{
                ...
                background:'#fdf6e3', border:'1px solid rgba(196,146,42,0.3)',
              ...
              {EMOJIS.map(e => (
                <button key={e}
                  onClick={() => { onReact(msgId, e); setShowMenu(false); }}
                  style={{ background:'none', border:'none', cursor:'pointer', fontSize:18, padding:'2px 4px', borderRadius:4, transition:'transform 0.1s' }}
              ...
              <button onClick={() => { onReply(msg); setShowMenu(false); }}
                style={{ background:'none', border:'none', cursor:'pointer', fontFamily:"'Space Mono',monospace", fontSize:9, color:'#9b6b2f', letterSpacing:'0.05em', padding:'2px 4px' }}>
                ↩ REPLY
              </button>
              {isMe && (
                <button onClick={() => { onDelete(msgId); setShowMenu(false); }}
                  style={{ background:'none', border:'none', cursor:'pointer', fontFamily:"'Space Mono',monospace", fontSize:9, color:'#c0392b', letterSpacing:'0.05em', padding:'2px 4px' }}>
                  ✕ DELETE
                </button>
              )}
              <button onClick={() => setShowMenu(false)}
                style={{ marginLeft:'auto', background:'none', border:'none', cursor:'pointer', fontFamily:"'Space Mono',monospace", fontSize:9, color:'#9b8b7a', padding:'2px 4px' }}>
                ESC
              </button>
            </motion.div>
          )}
      ...
      {msg.reactions?.length > 0 && (
        <div style={{ maxWidth:'78%', marginTop:2 }}>
          <ReactionBar reactions={msg.reactions} messageId={msgId} currentUserEmail={currentUserEmail} onReact={onReact} />
        </div>
      )}

      {/* Replies */}
      {msg.replies?.length > 0 && (
        <div style={{ maxWidth:'78%', width:'100%', marginTop:2 }}>
          <ReplyThread replies={msg.replies} currentUserEmail={currentUserEmail} onReact={onReact} />
        </div>
      )}

      <span style={{ fontFamily:"'Space Mono',monospace", fontSize:8, color:'#9b8b7a', opacity:0.4, marginTop:2 }}>
        double-tap to react
      </span>
    </motion.div>
  );
...
const TopicModal = ({ isOpen, onClose, onSelect, domainData }) => {
  ...
  const allEntries = [];
  if (domainData?.[activeTab]) {
    Object.values(domainData[activeTab]).forEach(items => items.forEach(item => allEntries.push(item)));
  }
    ...
    <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }}
      ...
      <motion.div initial={{ scale:0.9, y:20 }} animate={{ scale:1, y:0 }}
        ...
        style={{
          ...
          border:'2px solid rgba(196,146,42,0.3)', borderRadius:8,
        ...
        <div style={{ padding:'20px 24px 16px', borderBottom:'1px solid rgba(196,146,42,0.2)', background:'linear-gradient(to bottom,#ede0be,#e4d4a8)' }}>
          <div style={{ fontFamily:"'Space Mono',monospace", fontSize:10, color:'#c4922a', letterSpacing:'0.15em', marginBottom:4 }}>
            🌿 LINK A KNOWLEDGE ENTRY
          </div>
          ...
          <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search entries..."
            style={{ marginTop:10, width:'100%', padding:'8px 12px', border:'1px solid rgba(196,146,42,0.3)', borderRadius:4, fontFamily:"'Cormorant Garamond',serif", fontSize:14, background:'rgba(255,255,255,0.7)', outline:'none', boxSizing:'border-box' }}
          />
        </div>
        <div style={{ display:'flex', borderBottom:'1px solid rgba(196,146,42,0.2)', background:'rgba(196,146,42,0.06)' }}>
          {Object.entries(DOMAIN_CONFIG).map(([key, cfg]) => (
            <button key={key} onClick={() => setActiveTab(key)}
              style={{ flex:1, padding:'10px 4px', border:'none', cursor:'pointer', background: activeTab===key ? 'rgba(196,146,42,0.15)' : 'none', borderBottom: activeTab===key ? `2px solid ${cfg.color}` : '2px solid transparent', fontFamily:"'IM Fell DW Pica',serif", fontSize:13, color: activeTab===key ? cfg.accent : '#7b6b5a', transition:'all 0.2s' }}>
              {cfg.icon} {cfg.label}
            </button>
          ))}
        </div>
        <div style={{ flex:1, overflowY:'auto', padding:'8px 0' }}>
          {filtered.length === 0 && (
            <div style={{ textAlign:'center', padding:24, fontFamily:"'IM Fell DW Pica',serif", color:'#9b8b7a', fontSize:14 }}>No entries found</div>
          )}
          {filtered.map((item, i) => (
            <motion.button key={item.id||i} whileHover={{ x:4, background:'rgba(196,146,42,0.1)' }}
              onClick={() => { onSelect({ domain:activeTab, label:item.title }); onClose(); }}
              style={{ display:'block', width:'100%', padding:'10px 24px', background:'none', border:'none', cursor:'pointer', textAlign:'left', borderBottom:'1px solid rgba(196,146,42,0.08)', transition:'all 0.15s' }}>
              <div style={{ fontFamily:"'IM Fell DW Pica',serif", fontSize:14, color:'#2a1a08' }}>{item.title}</div>
              {item.description && <div style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:12, color:'#7b6b5a', marginTop:2 }}>{item.description}</div>}
            </motion.button>
          ))}
        </div>
        <div style={{ padding:'10px 24px', borderTop:'1px solid rgba(196,146,42,0.15)', textAlign:'right' }}>
          <button onClick={onClose} style={{ fontFamily:"'Space Mono',monospace", fontSize:10, color:'#9b8b7a', background:'none', border:'none', cursor:'pointer', letterSpacing:'0.1em' }}>✕ CLOSE</button>
        </div>
      </motion.div>
    </motion.div>
  );
...
const ImagePreviewStrip = ({ preview, onClear }) => (
  <motion.div initial={{ height:0, opacity:0 }} animate={{ height:'auto', opacity:1 }} exit={{ height:0, opacity:0 }}
    style={{ padding:'8px 16px', borderTop:'1px solid rgba(196,146,42,0.15)' }}>
...
const AttachedTopicStrip = ({ topic, onClear }) => (
  <motion.div initial={{ height:0, opacity:0 }} animate={{ height:'auto', opacity:1 }} exit={{ height:0, opacity:0 }}
    style={{ padding:'6px 16px', borderTop:'1px solid rgba(196,146,42,0.15)', display:'flex', alignItems:'center', justifyContent:'space-between', background:`${DOMAIN_CONFIG[topic.domain]?.color || '#c4922a'}0a` }}>
...
const ReplyPreviewStrip = ({ replyingTo, onClear }) => {
  ...
  return (
    <motion.div initial={{ height:0, opacity:0 }} animate={{ height:'auto', opacity:1 }} exit={{ height:0, opacity:0 }}
      style={{ padding:'8px 16px', borderTop:'1px solid rgba(196,146,42,0.2)', background:'rgba(196,146,42,0.06)', display:'flex', alignItems:'center', gap:10 }}>
...
const CommunityChat = ({ domainData }) => {
  // FIX: get token and user from AuthContext — token is separate, user has email/name
  const { user, token } = useAuth();
  const currentUserEmail = user?.email || '';
  const currentUserName  = user?.name  || 'You';

  const [messages,        setMessages]       = useState([]);
  const [localReactions,  setLocalReactions] = useState({}); // { [msgId]: [{emoji, users:[]}] }
  const [localReplies,    setLocalReplies]   = useState({}); // { [msgId]: [...replies] }
  ...
  useEffect(() => {
    ...
    socket.on('connect_error', () => { setConnected(false); setError('Connection lost. Retrying...'); });

    // FIX: history arrives as [{_id, user:{name,email}, text, timestamp}]
  ...
  const handleImageChange = (e) => {
    ...
    if (!file) return;
    // Convert to base64 so it can be sent via socket and shown to all users
  ...
  const handleSend = async () => {
    ...
    // Upload image to backend first if one is attached
    if (imageFile) {
      try {
        ...
        if (res.ok) {
          ...
          imageUrl = data.url; // e.g. "http://localhost:5000/uploads/filename.jpg"
        }
      } catch (err) {
    ...
    if (replyingTo) {
      ...
      setLocalReplies(prev => ({
        ...prev,
        [parentId]: [...(prev[parentId] || []), replyMsg],
      }));
      setReplyingTo(null);
    } else {
      // Build message payload — encode topic + image URL into text for storage
      // Format: __IMG__url__ENDIMG__ prefix if image present
      let txt = inputText.trim();
      if (attachedTopic) {
        txt = `__TOPIC__${attachedTopic.domain}__${attachedTopic.label}__END__${txt}`;
      }
      if (imageUrl) {
        txt = `__IMG__${imageUrl}__ENDIMG__${txt}`;
      }
  ...
  // ── ENRICH MESSAGES with local overlay data ────────────────────────────────
  const enrichedMessages = messages.map(msg => {
    const id = msg._id || msg.id;
    return {
      ...msg,
      reactions: localReactions[id] || [],
      replies:   localReplies[id]   || [],
    };
  });
  ...
  return (
    <>
      ...
      <div style={{
        display:'flex', flexDirection:'column',
        height:'calc(100vh - 200px)', maxWidth:760, margin:'0 auto',
        background:'linear-gradient(to bottom,#f8f2e0,#f0e8d0)',
        border:'1px solid rgba(196,146,42,0.25)', borderRadius:6,
        ...
        <div style={{ padding:'14px 20px', zIndex:1, borderBottom:'1px solid rgba(196,146,42,0.2)', background:'linear-gradient(to bottom,#ede0be,#e4d4a8)', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          <div>
            <div style={{ fontFamily:"'Space Mono',monospace", fontSize:9, color:'#c4922a', letterSpacing:'0.2em', marginBottom:2 }}>🌿 COMMUNITY CIRCLE</div>
            <div style={{ fontFamily:"'IM Fell DW Pica',serif", fontSize:20, color:'#2a1a08' }}>Living Memory — Open Discourse</div>
          </div>
          <div style={{ display:'flex', alignItems:'center', gap:6, fontFamily:"'Space Mono',monospace", fontSize:9, color:'#9b8b7a' }}>
            <div style={{ width:6, height:6, borderRadius:'50%',
              background: connected ? '#6db86d' : '#d05e52',
              boxShadow: `0 0 6px ${connected ? '#6db86d' : '#d05e52'}` }} />
            {connected ? `${onlineCount} scholars online` : 'reconnecting...'}
          </div>
        </div>

        {/* Signed in as */}
        <div style={{ padding:'6px 20px', zIndex:1, borderBottom:'1px solid rgba(196,146,42,0.1)', background:'rgba(196,146,42,0.04)', display:'flex', alignItems:'center', gap:8 }}>
          <Avatar name={currentUserName} email={currentUserEmail} size={20} />
          <span style={{ fontFamily:"'Space Mono',monospace", fontSize:9, color:'rgba(92,51,23,0.5)', letterSpacing:'0.05em' }}>
            Signed in as <span style={{ color:'rgba(92,51,23,0.8)' }}>{currentUserName}</span>
          </span>
        </div>
        ...
        <div ref={scrollRef} style={{ flex:1, overflowY:'auto', padding:'20px 24px', zIndex:1, scrollbarWidth:'thin', scrollbarColor:'rgba(196,146,42,0.3) transparent' }}>
          ...
          {messages.length === 0 && connected && (
            <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', height:'100%', gap:10, paddingTop:40, textAlign:'center' }}>
              <div style={{ fontSize:40 }}>🌿</div>
              <div style={{ fontFamily:"'IM Fell DW Pica',serif", fontSize:18, color:'rgba(155,107,47,0.45)', fontStyle:'italic' }}>
                Be the first to share wisdom today
              </div>
            </div>
          )}
          ...
          {Object.entries(grouped).map(([date, msgs]) => (
            <div key={date}>
              <div style={{ display:'flex', alignItems:'center', gap:12, margin:'14px 0 10px', fontFamily:"'Space Mono',monospace", fontSize:8, letterSpacing:'.15em', color:'rgba(155,107,47,0.4)', textTransform:'uppercase' }}>
                <div style={{ flex:1, borderTop:'1px solid rgba(155,107,47,0.15)' }}/>
                <span style={{ background:'rgba(196,146,42,0.07)', border:'1px solid rgba(196,146,42,0.15)', borderRadius:20, padding:'2px 10px' }}>{date}</span>
                <div style={{ flex:1, borderTop:'1px solid rgba(155,107,47,0.15)' }}/>
              </div>
              {msgs.map(msg => (
                <MessageCard key={msg._id || msg.id}
                  msg={msg}
        ...
        {/* Error banner */}
        {error && (
          <div style={{ padding:'6px 20px', background:'rgba(208,94,82,0.08)', borderTop:'1px solid rgba(208,94,82,0.15)', zIndex:1, fontFamily:"'Space Mono',monospace", fontSize:9, color:'#c05040', textAlign:'center' }}>
            ⚠ {error}
          </div>
        )}
        ...
        <div style={{ zIndex:1, borderTop:'2px solid rgba(196,146,42,0.15)', background:'linear-gradient(to top,#ede0be,#f0e8d0)' }}>
          <AnimatePresence>
            {replyingTo    && <ReplyPreviewStrip   replyingTo={replyingTo}     onClear={() => setReplyingTo(null)} />}
          </AnimatePresence>
          <AnimatePresence>
            {attachedTopic && <AttachedTopicStrip  topic={attachedTopic}       onClear={() => setAttachedTopic(null)} />}
          </AnimatePresence>
          <AnimatePresence>
            {imagePreview  && <ImagePreviewStrip   preview={imagePreview}      onClear={() => { setImageFile(null); setImagePreview(null); }} />}
          </AnimatePresence>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:10, padding:'10px 16px' }}>
            ...
            {/* Image attach */}
            <label title="Attach image" style={{ cursor:'pointer', color:'#9b8b7a', transition:'color 0.2s', flexShrink:0 }}
            ...
            {/* Topic attach */}
            <button title="Link a knowledge entry" onClick={() => setTopicModalOpen(true)}
              style={{ background:'none', border:'none', cursor:'pointer', flexShrink:0, color: attachedTopic ? '#c4922a' : '#9b8b7a', transition:'color 0.2s' }}
              onMouseEnter={e => e.currentTarget.style.color='#c4922a'}
              onMouseLeave={e => e.currentTarget.style.color= attachedTopic ? '#c4922a' : '#9b8b7a'}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            ...
            <input ref={inputRef} type="text" value={inputText}
              ...
              placeholder={
                ...
                replyingTo      ? `Reply to ${resolveUser(replyingTo.user).name}...` :
                attachedTopic   ? `Speak about ${attachedTopic.label}...` :
                                  "Share your wisdom with the circle..."
              }
              disabled={!connected}
              style={{ flex:1, background:'transparent', border:'none', outline:'none',
                fontFamily:"'Cormorant Garamond',serif", fontSize:15, color:'#353535',
                opacity: connected ? 1 : 0.5 }}
            />

            <motion.button whileTap={{ scale:0.93 }} onClick={handleSend} disabled={!canSend}
              style={{
                background: canSend ? 'linear-gradient(135deg,#c4922a,#9b6b2f)' : 'rgba(196,146,42,0.2)',
                border:'none', borderRadius:6, padding:'8px 16px',
                cursor: canSend ? 'pointer' : 'not-allowed',
                fontFamily:"'Space Mono',monospace", fontSize:10,
                color: canSend ? '#fff' : '#9b8b7a',
                letterSpacing:'0.1em', fontWeight:700, transition:'all 0.2s',
                boxShadow: canSend ? '0 2px 8px rgba(196,146,42,0.3)' : 'none',
              }}>
              SEND
            </motion.button>
          </div>

          <div style={{ padding:'4px 16px 8px', fontFamily:"'Space Mono',monospace", fontSize:8, color:'rgba(155,107,47,0.4)', letterSpacing:'0.08em', display:'flex', gap:16 }}>
            <span>🌿 double-tap any message to react</span>
            <span>📎 link knowledge entries with the chain icon</span>
          </div>
        </div>
      </div>
    </>
